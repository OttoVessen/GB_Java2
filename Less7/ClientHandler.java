...
import static Less7.MessagePatterns.MESSAGE_SEND_PATTERN;
            ...
            public void run() {
                while (!Thread.currentThread().isInterrupted()) {
                    try {
                        ...
                        // Нашел описание метода split и сделал через него, вроде работает)))
                        String[] messArgs = msg.split(" ",3);
                        if (messArgs[0].equals("/w")){
                        String userTo = messArgs[1];
                        String message = messArgs[2];
